...
public class CommandTerminal implements EventListener {
    ...
    private ExecutorService executorService
            = Executors.newCachedThreadPool(new ThreadFactoryBuilder().setNameFormat("commanding-pool-%d").build());
    ...
    public void start(boolean commandable, ExtendedLogger logs, ConsoleReader reader) {
        ...
        this.terminalTask = new TerminalTask(reader, s -> {
            ...
            if(!CommandRegistry.getInstance().hasCommand(command)) {
                ...
                executorService.execute(() -> CommandRegistry.getInstance().executeCommand(args,
